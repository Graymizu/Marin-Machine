...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void DrawBody(IReadOnlyDictionary<JointType, Joint> joints, IDictionary<JointType, Point> jointPoints, DrawingContext drawingContext, Pen drawingPen)
        {
            ...
            foreach (JointType jointType in joints.Keys)
            {
                ...
                if (drawBrush != null)
                {
                    ...
                    drawingContext.DrawEllipse(drawBrush, null, jointPoints[jointType], 3, 3);//GetJointThickness(jointIndexs[jointType]), GetJointThickness(jointIndexs[jointType]));
                }
            }
        }
        ...
        private void GetData_Click(object sender, RoutedEventArgs e)
        {
            ...
            string[] arr = Array.ConvertAll<double, string>(jointAngles, Convert.ToString);
            /*for(int i =0; i < arr.Length; i++)
            {
                Console.WriteLine(arr[i]);
            }*/
            //Console.WriteLine(Array.ConvertAll<double, string>(jointAngles, Convert.ToString));
            //Console.WriteLine(Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\..\")));
            System.IO.File.AppendAllLines(Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\..\")) + @"\DanceData.txt", arr);
        }
